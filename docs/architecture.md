...
Production Workspace UI + ops hardening (M7A): `packages/production-workspace-contracts`, `packages/production-workspace-services`, `src/better-chat-cut/production-workspace/`, same-origin `/api/better-chat-cut/workspace/*`, MCP eight `workspace_*` tools. Docs: `docs/production-workspace*.md`, migrations/diagnostics. Next: M7B distribution/OAuth/backup.
