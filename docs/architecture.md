...
| `packages/video-plans/` | VideoPlanV1 schema, hash, deterministic schedule (M5A) |
| `packages/project-video-assembly/` | Timeline assembly, inspection, TimelineComposition render validation (M5A) |
...
Production Workspace UI + ops hardening (M7A): `packages/production-workspace-contracts`, `packages/production-workspace-services`, `src/better-chat-cut/production-workspace/`, same-origin `/api/better-chat-cut/workspace/*`, MCP eight `workspace_*` tools. Docs: `docs/production-workspace*.md`, migrations/diagnostics.

Desktop distribution + OAuth + backup/restore + RC (M7B): `packages/desktop-distribution-contracts`, `packages/desktop-distribution`, `packages/secure-connection-onboarding`, `packages/workspace-backup-restore`, `packages/release-candidate-qualification`, same-origin distribution/connections/backups/restores/release-candidates APIs, nineteen finalization MCP tools. Roots: `BETTER_CHAT_CUT_DISTRIBUTION_ROOT`, `BETTER_CHAT_CUT_BACKUP_ROOT`, `BETTER_CHAT_CUT_RELEASE_CANDIDATE_ROOT`. Explicit manual update policy; encrypted connection vault; roadmap closure gate.
